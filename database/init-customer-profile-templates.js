...
async function insertTemplates() {
    ...
    try {
        ...
        // 插入或更新模板
        console.log('📥 正在插入/更新模板数据...\n');

        for (const template of templates) {
            try {
                // 检查模板是否已存在
                const [existing] = await connection.execute(
                    'SELECT id FROM customer_profile_templates WHERE template_code = ?',
                    [template.template_code]
                );

                const sql = existing.length > 0
                    ? `UPDATE customer_profile_templates SET
                        template_name = ?,
                        description = ?,
                        fields = ?,
                        field_groups = ?,
                ...
                const values = existing.length > 0
                    ? [
                ...
                console.log(`  ✓ ${template.template_name} (${existing.length > 0 ? '已更新' : '已插入'})`);
            } catch (error) {
                console.error(`  ✗ ${template.template_name} 失败: ${error.message}`);
            }
        }
        ...
        // 查询并显示结果
        const [rows] = await connection.execute('SELECT id, template_code, template_name FROM customer_profile_templates ORDER BY is_default DESC, created_at DESC');
        console.log('====================================');
        console.log(`  共有 ${rows.length} 个模板`);
        console.log('====================================\n');

        rows.forEach((row, index) => {
            console.log(`${index + 1}. [ID: ${row.id}] ${row.template_name}`);
            console.log(`   编码: ${row.template_code}`);
        });

        console.log('\n✅ 模板初始化完成，现在可以使用客户资料功能！');
