...
**推荐方案**：Node.js + Express（考虑到前端已使用JavaScript）
...
- **关系型数据库**：MySQL 8.0（阿里云RDS）
- **缓存数据库**：Redis 6.0+（会话、热点数据）
- **文件存储**：阿里云OSS（客户照片、案例图片）
...
1. **多租户隔离**：通过 `org_id`（机构ID）字段实现数据隔离
2. **软删除**：使用 `is_deleted` 字段，不物理删除数据
3. **审计追踪**：所有表包含 `created_at`, `updated_at`, `created_by`, `updated_by`
4. **字符集**：utf8mb4（支持emoji和特殊字符）
5. **引擎**：InnoDB（支持事务、外键）
6. **索引策略**：
   - 主键：自增ID
   - 业务唯一键：唯一索引
...
```
organizations (机构)
    ├── 1:N → organizations (子机构)
    ├── 1:N → users (员工)
    ├── 1:N → customers (客户)
    ├── 1:N → orders (订单)
    ├── 1:N → tasks (任务)
    └── 1:N → solution_templates (方案模板)

users (用户)
    ├── M:N → roles (角色) [通过 user_roles]
    ├── 1:N → customers (作为顾问)
    ├── 1:N → tasks (作为执行人)
    └── 1:N → operation_logs (操作日志)

roles (角色)
    └── M:N → permissions (权限) [通过 role_permissions]

customers (客户)
    ├── 1:N → customer_diagnoses (诊断记录)
    ├── 1:N → customer_cases (客户案例)
    ├── 1:N → orders (订单)
    └── 1:N → tasks (任务)
...
GET     /api/v1/customers/123          # 获取客户详情
POST    /api/v1/customers              # 创建客户
PUT     /api/v1/customers/123          # 更新客户
DELETE  /api/v1/customers/123          # 删除客户
GET     /api/v1/customers/123/orders   # 获取客户的订单列表
```
...
```javascript
// 获取客户列表
GET /api/v1/customers?page=1&pageSize=20&keyword=张三&memberLevel=gold
...
| 角色编码 | 角色名称 | 数据权限范围 | 说明 |
|---------|---------|------------|------|
| `super_admin` | 超级管理员 | all | 总部最高权限，查看所有数据 |
| `platform_admin` | 平台管理员 | all | 总部管理员，管理加盟商 |
| `franchisee_admin` | 加盟商管理员 | org | 管理本机构所有门店 |
| `store_manager` | 门店店长 | store | 管理本门店数据 |
| `counselor` | 美容顾问 | self | 查看分配给自己的客户 |
| `beautician` | 美容师 | self | 查看分配给自己的订单 |
| `finance` | 财务人员 | org | 查看财务相关数据 |
...
franchisees:view            -- 查看加盟商
franchisees:manage          -- 管理加盟商

templates:view              -- 查看方案模板
templates:create            -- 创建方案模板
templates:edit              -- 编辑方案模板
...
const key = process.env.ENCRYPT_KEY; // 32字节密钥
...
// ❌ 错误示例（拼接SQL）
const sql = `SELECT * FROM customers WHERE phone = '${phone}'`; // 危险!
```
...
// 前端显示时转义
document.getElementById('name').textContent = customerName; // 安全
// document.getElementById('name').innerHTML = customerName; // 危险!
```
...
| 组件 | 规格 | 说明 |
|-----|------|------|
| **应用服务器** | 阿里云ECS 2核4G × 2 | 负载均衡 |
| **数据库** | 阿里云RDS MySQL 8.0 (4核8G) | 主从架构 |
| **缓存** | 阿里云Redis 6.0 (1G) | 会话、热点数据 |
| **存储** | 阿里云OSS 标准型 | 图片、文件存储 |
| **负载均衡** | 阿里云SLB | HTTPS, 健康检查 |
